...
${JSON.stringify(rows, null, 2)}`;
    ...
    async processWithGemini(systemPrompt, userPrompt, originalRows) {
        if (!this.geminiClient)
    ...
    async processWithOpenAI(systemPrompt, userPrompt, originalRows) {
    ...
    parseAIResponse(data, originalRows) {
        const imported = [];
        const skipped = [];
            ...
            for (const [key, value] of Object.entries(row)) {
                ...
                if (commonEmailKeys.some(x => k.includes(x))) {
                    if (!email)
                        ...
                        notes += ` Secondary Email: ${v};`;
                }
                else if (commonPhoneKeys.some(x => k.includes(x))) {
                    if (!phone)
                        phone = v;
                    else
                        notes += ` Secondary Phone: ${v};`;
                }
                    ...
                    const matchedSource = ['leads_on_demand', 'meridian_tower', 'eden_park', 'varah_swamy', 'sarjapur_plots'].find(s => v.toLowerCase().includes(s.replace('_', ' ')) || s.includes(v.toLowerCase()));
                    if (matchedSource)
